...
<video controls width="100%" preload="metadata" style="height: auto;">
...
File `src.c` là một chương trình C đọc input từ `stdin`, sau đó truyền buffer vào các hàm parser nhỏ. Mỗi hàm chứa một lỗi bộ nhớ khác nhau.
...
DGF cần biết trước các vị trí đích trong chương trình để có thể tập trung dồn năng lượng khi bước sang pha khai thác. Các vị trí này được mô tả trong file `BBtargets.txt`, mỗi dòng có định dạng:
...
Mục tiêu của quá trình tiền xử lý nhằm thu thập metadata cho AFLGo bao gồm:
...
`0x33` là ký tự `3`, `0x3a` là `:`. Input có dạng gần với:
...
| 3  | `3:<payload>`     | Heap Buffer Overflow | `bug_heap_overflow_length_field` | Heap Buffer Overflow |
